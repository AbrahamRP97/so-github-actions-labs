...
echo "✓ Directorios creados en: $WORK_DIR"
ls -la "$WORK_DIR"
echo ""
...
Parent PID: $PPID
EOF
echo "✓ Archivo execution.log creado"
echo ""
...
echo "✓ Cambiado a 644 (rw-r--r--)"
ls -l "$WORK_DIR/data/info.txt"

chmod 600 "$WORK_DIR/logs/execution.log"
echo "✓ Cambiado a 600 (rw-------)"
...
echo "PID: $$"
for i in {1..5}; do
    echo "Iteración $i - $(date)" >> /tmp/background-output.txt
    sleep 1
done
...
BG_PID=$!
echo "✓ Proceso iniciado con PID: $BG_PID"
...
wait $BG_PID
EXIT_CODE=$?
echo "✓ Proceso terminado con código: $EXIT_CODE"
echo ""
...
if [ -n "$SECRET_KEY" ]; then
    echo "✓ Secret key detectado: ${SECRET_KEY:0:4}****"
else
    echo "⚠ No se encontró SECRET_KEY"
fi

if [ -n "$CUSTOM_VAR" ]; then
    echo "✓ Variable personalizada: $CUSTOM_VAR"
else
    echo "⚠ No se encontró CUSTOM_VAR"
fi
echo ""
...
echo "✓ Artifacts copiados a ./artifacts/"
ls -lh ./artifacts/
echo ""
